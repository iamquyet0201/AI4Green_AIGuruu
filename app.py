...
# 🔑 API Keys
# IMPORTANT: Do NOT hardcode keys. Set GEMINI_API_KEY in Hugging Face Spaces Secrets.
...
MAX_IMAGE_SIDE = int(os.getenv("MAX_IMAGE_SIDE", "2048"))  # Max dimension
MIN_IMAGE_SIDE = int(os.getenv("MIN_IMAGE_SIDE", "100"))   # Min dimension
...
def optimize_image(content: bytes) -> Tuple[Optional[Image.Image], Optional[str]]:
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "quota" in error_msg or "rate limit" in error_msg or "429" in error_msg:
                logger.error("❌ Gemini quota exceeded. Disabling Gemini for this session.")
                GEMINI_AVAILABLE = False
                return None
...
async def health():
    """
    Light health endpoint for monitors.
    Must be fast and must not run inference.
    """
    return {
        "ok": True,
        "yolo_loaded": bool(model),
        "gemini_configured": bool(GEMINI_MODEL) and bool(GEMINI_AVAILABLE),
    }

# Accept both /predict and /predict/ so existing clients keep working
@app.post("/predict")
@app.post("/predict/")
    ...
    if USE_PARALLEL_EXECUTION:
        try:
            (yolo_counts, detections), gemini_items = await asyncio.gather(
                asyncio.to_thread(run_yolo_detection, img),
                scan_gemini_labels(img),
            )
        except Exception as e:
...
if __name__ == "__main__":
    ...
    # Local run only. On Hugging Face, Docker CMD will run uvicorn.
